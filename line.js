  ...
  constructor (size, centArr) {
    ...
    this.group.quaternion.onChange(() => {
      ...
      for (let i = 0; i < this.gongDegs.length; i++) {
        if (this.degs > (this.gongDegs[i] - this.degreeIncrement) && this.degs < (this.gongDegs[i] + this.degreeIncrement) && this.gongYet[i] === false){
          this.gongYet[i] = true;
          console.log("gong" + i);
        }
        if (this.degs > (this.gongDegs[i] + this.degreeIncrement + 3)){ this.gongYet[i] = false}
      }
